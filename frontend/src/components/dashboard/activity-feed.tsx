...
export function ActivityFeed({ workspaceId }: { workspaceId: string }) {
    ...
    return (
        <Card className="glass-card">
            ...
            <CardContent className="h-[300px] pl-2 pr-4">
                {/* Adjust padding for scrollbar */}
                <ScrollArea className="h-full w-full pr-4">
                    {activities.length === 0 ? (
                        <div className="flex flex-col items-center justify-center h-full text-muted-foreground py-8">
                            <MoreHorizontal className="h-8 w-8 mb-2 opacity-50" />
                        ...
                        <div className="space-y-5">
                            {activities.map((log) => (
                                <div key={log.id} className="flex gap-3 relative group">
                                    {/* Timeline line */}
                                    <div className="absolute left-[18px] top-10 bottom-[-20px] w-[1px] bg-border group-last:hidden" />
                                    ...
                                    <div className="flex-1 space-y-1">
                                        ...
                                        <div className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full bg-secondary/50 text-xs text-muted-foreground w-fit mt-1">
                                            {getIcon(log.action)}
